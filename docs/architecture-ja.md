...
ブラウザ
  ...
  │     ├── react-router-dom  — クライアントサイドルーティング（12 ルート）
  │     ├── zustand           — authStore（user, tenants, authenticated）
  │     ├── tramli            — authFlowDefinition（10 ステート）
  │     └── tramli-react      — useAuthFlow フック
  │
  └── nginx（ポート 80）
        ├── /                 → SPA（index.html フォールバック）
...
UNAUTHENTICATED（初期状態）
  ...
  ▼
LOGIN_REDIRECT
  │ Auto: RedirectToLogin（noop — UI がリダイレクトを実行）
  ▼
LOGIN_PENDING
  │ External: IdpCallbackGuard（URL の ?code=&state= を待機）
  ▼
CALLBACK_RECEIVED
...
```
CHECKING（初期状態）
  │ External: SessionResumeGuard → api.me() + api.myTenants()（並列）
  ├─ 成功 ──→ AUTHENTICATED（終端）
  └─ エラー ──→ NO_SESSION（終端）
```
...
```
zustand authStore
  ├── user          — ResumeUser（FlowContext）から
  ├── tenants       — ResumeTenants（FlowContext）から
  ├── authenticated — sessionResumeDefinition が AUTHENTICATED に達したとき true
  └── loading       — CHECKING 状態がアクティブな間 true
```
...
App.jsx
  ├── Sidebar.jsx
  │     └── [Monitor リンク — ADMIN/OWNER のみ]
  ├── AuthFlowStatus.jsx     （Dashboard）
  ├── ServerDataTable.jsx    （Users, Sessions, Audit, Members, Invitations）
  │     └── DateRangeFilter.jsx（Audit のみ）
  └── DataTable.jsx          （IdpConfig, Webhooks, SigningKeys — クライアントサイド）
```
