...
graph TD
    HTML["index.html"]
    MAIN["src/main.jsx<br/>BrowserRouter + StrictMode ラップ"]
    APP["src/App.jsx<br/>認証ガード + Routes 定義"]
...
```mermaid
graph TD
    MON["Monitor()<br/>デフォルトエクスポート (MonitorView のラッパー)"]
    VIEW["MonitorView()<br/>実際のロジックとレイアウト"]
    FC["FlowCard()<br/>各フロー定義カード"]
    LF["LiveFeed()<br/>リアルタイムイベントリスト"]
...
```mermaid
graph TD
    ROOT["volta-auth-console/"]

    ROOT --> F1["index.html<br/>Vite エントリ HTML"]
    ROOT --> F2["vite.config.js<br/>Vite 設定 (proxy, base, port)"]
    ROOT --> F3["nginx.conf<br/>本番 nginx 設定"]
    ROOT --> F4["eslint.config.js<br/>ESLint フラット設定"]
    ROOT --> F5["package.json<br/>依存定義 (v0.2.0)"]
    ROOT --> F6["package-lock.json"]
    ROOT --> D_PUBLIC["public/<br/>静的アセット (favicon 等)"]
    ROOT --> D_DIST["dist/<br/>ビルド成果物 (git 管理外)"]

    ROOT --> D_DOCS["docs/<br/>追加ドキュメント"]
    D_DOCS --> DM1["architecture.md / architecture-ja.md"]
    D_DOCS --> DM2["getting-started.md / getting-started-ja.md"]
    D_DOCS --> DM3["pages.md / pages-ja.md"]
    D_DOCS --> DM4["monitor-page.md / monitor-page-ja.md"]
    D_DOCS --> DM5["spec.md<br/>旧 spec (docs 配下)"]

    ROOT --> D_SPEC["spec/"]
    D_SPEC --> SM["SPEC.md<br/>本ドキュメント (正式 spec)"]

    ROOT --> D_SRC["src/"]
    D_SRC --> S1["main.jsx<br/>BrowserRouter + StrictMode エントリ"]
    D_SRC --> S2["App.jsx<br/>認証ガード + Routes"]
    D_SRC --> S3["index.css<br/>グローバルスタイル (Tailwind @import)"]
    D_SRC --> D_ASSETS["assets/<br/>バンドル対象アセット"]

    D_SRC --> D_COMP["components/"]
    D_COMP --> C1["Sidebar.jsx<br/>ナビゲーションサイドバー"]
    D_COMP --> C2["DataTable.jsx<br/>クライアントサイドテーブル"]
    D_COMP --> C3["ServerDataTable.jsx<br/>サーバーサイドページネーションテーブル"]
    D_COMP --> C4["AuthFlowStatus.jsx<br/>tramli 状態バッジ"]
    D_COMP --> C5["DateRangeFilter.jsx<br/>日時範囲フィルター"]

    D_SRC --> D_HOOKS["hooks/"]
    D_HOOKS --> H1["useAuthFlow.js<br/>tramli session-resume → zustand 同期"]
    D_HOOKS --> H2["usePaginatedQuery.js<br/>URL 同期ページネーションフック"]

    D_SRC --> D_LIB["lib/"]
    D_LIB --> L1["api.js<br/>API クライアント (fetch ラッパー)"]

    D_SRC --> D_PAGES["pages/"]
    D_PAGES --> P1["Dashboard.jsx"]
    D_PAGES --> P2["Users.jsx"]
    D_PAGES --> P3["Tenants.jsx"]
    D_PAGES --> P4["Members.jsx"]
    D_PAGES --> P5["Invitations.jsx"]
    D_PAGES --> P6["Sessions.jsx"]
    D_PAGES --> P7["Audit.jsx"]
    D_PAGES --> P8["IdpConfig.jsx"]
    D_PAGES --> P9["Webhooks.jsx"]
    D_PAGES --> P10["SigningKeys.jsx"]
    D_PAGES --> P11["Settings.jsx"]
    D_PAGES --> P12["Monitor.jsx"]

    D_SRC --> D_STORE["store/"]
    D_STORE --> ST1["authStore.js<br/>Zustand 認証ストア"]
    D_STORE --> ST2["authFlowDefinition.js<br/>tramli フロー定義 (OIDC + resume)"]
```
...
graph TB
    ...
    subgraph Sidebar["Sidebar (w-56)"]
        ...
        S12["Monitor /monitor<br/>(ADMIN/OWNER only)"]
    end

    subgraph Pages["Pages (flex-1 p-6)"]
        P1["Dashboard<br/>統計カード×4<br/>AuthFlowStatus"]
        P2["Users<br/>ServerDataTable<br/>ResetMFA"]
        P3["Tenants<br/>DataTable<br/>Suspend/Activate<br/>MFA toggle"]
        P4["Members<br/>ServerDataTable<br/>ロール色分け"]
        P5["Invitations<br/>ServerDataTable<br/>ステータスフィルター"]
        P6["Sessions<br/>ServerDataTable<br/>Revoke"]
        P7["Audit<br/>ServerDataTable<br/>DateRangeFilter"]
        P8["IdpConfig<br/>DataTable (read-only)"]
        P9["Webhooks<br/>CRUD<br/>配信履歴"]
        P10["SigningKeys<br/>DataTable<br/>Rotate Key"]
        P11["Settings<br/>Profile / MFA / Sessions"]
        P12["Monitor<br/>SSE Feed<br/>VizDashboard (lazy)"]
    end
...
stateDiagram-v2
    ...
    state "authFlowDefinition<br/>(volta-auth-oidc  TTL:10m)" as OIDCFlow {
        [*] --> UNAUTHENTICATED
    ...
    state "sessionResumeDefinition<br/>(session-resume  TTL:30s)" as ResumeFlow {
        [*] --> CHECKING
